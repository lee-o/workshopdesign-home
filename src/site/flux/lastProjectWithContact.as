package site.flux 
{
	 ...
	 * @author lee-o
	 */
	public class lastProjectWithContact extends Sprite
	{
		...
		//
		//
		public function lastProjectWithContact(_xml:XML,_largeur:uint = 400,_hauteur:uint = 400) 
		{
			this.visible = false;
			this.alpha = 0;
			//
		...
		private function project_Loaded(e:Event = null):void
		{
			...
			for (var item:String in conteneurProjectXml.project) {
				...
				//
				description_txt.condenseWhite = true;
				//
				...
				{
					lien_txt.y = description_txt.y
				}else {
					if (description_txt.numLines > 1)
					{
						lien_txt.y = description_txt.y + description_txt.height + 2;
					}else {
						lien_txt.y = description_txt.y + description_txt.height - 2;
					}
				}
		...
		private function buildConteneur_txt():void
		{
			...
			if (conteneur_txt.height > maskConteneur_txt.height) {
				...
				//MainWorkshopDesign.scrollVector.push(sc);
				addChild(sc);
			...
			if(sc){
				...
				scrollRect = new Rectangle(0, 0, conteneur_txt.width, reelHeight);
			}
			//
			//le module est construit on renvoi u complete 
			TweenMax.delayedCall(0.3, dispatchCustomEvent );
		}
		...
		private function resize():void
		{
			for (var i:uint = 0; i < conteneur_txt.numChildren; i++) {
				...
				if (_text_mc.getChildByName("trait")) {
