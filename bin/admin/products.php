...
$sql = "SELECT COUNT(*) FROM _products WHERE id > 1";
...
$references = $bdd->query('SELECT * FROM _products order by ordre ASC');
//Ajout d'une fiche projet
?>
<div id="addButton">
		<h2 id="titleMore"><img id="morePict" src="images/moreW.png" width="10" height="10" border="none"/>
        <a href="addProducts.php?nbrMax=<?php echo $nbrEntree;?>" target="_self">Ajouter une fiche produit</a></h2>
</div>
<?php
//
...
while ($donnees = $references->fetch())
{
	if($donnees['id'] == 1){
		...
		echo "<div id=\"ficheContact\">";
			echo "<h2 id=\"title\">";
				echo "<FORM name=\"ordreForm\" method=\"post\" action=\"upDateOrdreProducts.php\">";
					...
					echo "<div id=\"interligne\"></div>";
				echo "</FORM>";
				echo "<FORM method=\"post\" action=\"upDateProducts.php\" enctype=\"multipart/form-data\" onSubmit=\"\">";
