...
if($_POST['supprimer'] == "Supprimer" && $_POST['ID'] != ''){
	...
	$ord = mysql_query("SELECT ordre FROM _liens WHERE id = $id") or die('error : '.mysql_error());
	$ordNum = mysql_fetch_array($ord);
	$delOrd = $ordNum['ordre'];
	
	
	$result = mysql_query('DELETE FROM _liens WHERE id = \''.$id.'\'') or die('error : '.mysql_error());
	if (!$result) {
		...
		*/
		...
		$reOrd = mysql_query("UPDATE _liens SET ordre=ordre-1 WHERE ordre > $delOrd") or die('error : '.mysql_error());
		//
		redirige("liens.htm");
	}
	mysql_close($db);
