...
if($_POST['modifier'] == "ordre" && $_POST['ID'] != ''){
	...
	if($oldPos < $newPos){
		$result=mysql_query("SELECT id,ordre FROM _works WHERE ordre > $oldPos && ordre <= $newPos") or die('error : '.mysql_error());
		while($row = mysql_fetch_array($result)){
			$modifPos = mysql_query('UPDATE _works SET ordre = \''.($row['ordre'] - 1).'\'  WHERE id = \''.$row['id'].'\'') or die('error : '.mysql_error());
		}
		$modifPos = mysql_query('UPDATE _works SET ordre = \''.$newPos.'\'  WHERE id = \''.$ID.'\'') or die('error : '.mysql_error());
		if (!$modifPos) {
