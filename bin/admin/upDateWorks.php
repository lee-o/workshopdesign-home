...
if($_POST['modifier'] == "Enregistrer" && $_POST['ID'] != ''){
	...
	if($photo != ''){	
		$result = mysql_query('UPDATE _works SET name = \''.$name.'\',link = \''.$link.'\', visible = \''.$visible.'\', picture = \''.$photo.'\'  WHERE id = \''.$id.'\'') or die('error : '.mysql_error());
	}else{
		$result = mysql_query('UPDATE _works SET name = \''.$name.'\',link = \''.$link.'\', visible = \''.$visible.'\'  WHERE id = \''.$id.'\'') or die('error : '.mysql_error());
	}
	if (!$result) {
