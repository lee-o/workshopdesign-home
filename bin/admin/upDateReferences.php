...
if($_POST['modifier'] == "Enregistrer" && $_POST['ID'] != ''){
	...
	$production = stripDC($_POST['production']);
	$lienProduction = stripDC($_POST['lienProduction']);
	$realisation = stripDC($_POST['realisation']);
	$lienRealisation = stripDC($_POST['lienRealisation']);
	...
	if($photo['name'] != ''){	
		...
		if ( $upImg ) {
			...
			$result = mysql_query('UPDATE _references SET name = \''.$name.'\',type = \''.$type.'\',link = \''.$link.'\', production = \''.$production.'\', lienProduction = \''.$lienProduction.'\', realisation = \''.$realisation.'\', lienRealisation = \''.$lienRealisation.'\' , description = \''.$desc.'\', visible = \''.$visible.'\', picture = \''.$repFinal.$upImg.'\'  WHERE id = \''.$id.'\'') or die('error : '.mysql_error());
			if (!$result) {
		...
		$db = mysql_connect('mysql5-22.perso', 'workshopmbdd', '2F0BzY9iKRFi');
		mysql_select_db('workshopmbdd', $db);	
		$result = mysql_query('UPDATE _references SET name = \''.$name.'\',type = \''.$type.'\',link = \''.$link.'\', production = \''.$production.'\', lienProduction = \''.$lienProduction.'\', realisation = \''.$realisation.'\', lienRealisation = \''.$lienRealisation.'\' , description = \''.$desc.'\', visible = \''.$visible.'\'  WHERE id = \''.$id.'\'') or die('error : '.mysql_error());
		if (!$result) {
			die('Invalid query: ' . mysql_error());
		}else{
			redirige("references.htm");
